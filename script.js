...
window.addEventListener("scroll", () => {
    if (window.scrollY > 0 && window.innerWidth > 500) {
        document.querySelector(".logo").classList.add("short");
        document.querySelector(".l-name").classList.add("d-none");
    } else {
        document.querySelector(".logo").classList.remove("short");
        setTimeout(() => {
            document.querySelector(".l-name").classList.remove("d-none");
        }, 100)
